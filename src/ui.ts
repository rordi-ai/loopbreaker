...
function render(s) {
 ...
 const behaviors=s.behaviors.map(b=>'<div class="node"><div class="row"><strong>'+esc(b.id)+'</strong><span class="status '+b.status+'">'+esc(b.status)+(b.enforced?' · enforced':' · advisory')+'</span></div><div>'+esc(b.title)+'</div><div class="tiny">evidence: '+(b.evidence_ids.length?b.evidence_ids.map(esc).join(', '):'none')+(b.waiver_id?' · waiver: '+esc(b.waiver_id):'')+'</div></div>').join('');
