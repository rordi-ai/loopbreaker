...
description: Implement a feature from an existing frozen Loopbreaker behavior contract. Use when coding should begin, continue, or be repaired against named behaviors, with minimal scope, production wiring, repository-native verification, and evidence recorded without allowing implementation to redefine acceptance.
---
...
1. Call `review_substrate` for the issue.
2. Confirm the requested work maps to named behavior IDs.
3. Read repository instructions and the production construction path.
4. Inspect current status and preserve unrelated worktree changes.

If there is no imported contract, stop and use `$plan-feature`. If the requested
change would add acceptance requirements, report the scope change instead of coding
it as an incidental improvement.
...
status. Never claim done solely because tests pass or review converged.
